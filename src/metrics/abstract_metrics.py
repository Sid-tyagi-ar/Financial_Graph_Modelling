...
class SumExceptBatchKL(Metric):
    ...
    def update(self, p, q) -> None:
        self.total_value += F.kl_div(p, q, reduction='sum')
...
class CrossEntropyMetric(Metric):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        output = F.cross_entropy(preds, target, reduction='sum', weight=self.weight)
...
class DummySamplingMetrics(torch.nn.Module):
    def __init__(self, dataloaders):
        super().__init__()

    def forward(self, generated_graphs, name, current_epoch, val_counter, test):
        pass

    def reset(self):
        pass

    def log_epoch_metrics(self, current_epoch, val_counter, test):
        pass