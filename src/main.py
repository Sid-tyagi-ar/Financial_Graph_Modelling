...
def setup_wandb(cfg):
    ...
    wandb.login(key="260293a18eab7458fcafaf31ce7792932c6d6089")
...
def main(cfg: DictConfig):
    ...
    if dataset_config["name"] in ['sbm', 'comm-20', 'planar']:
        ...
        visualization_tools = NonMolecularVisualization(dataset_infos=dataset_infos)
        ...
        model_kwargs = {'dataset_infos': dataset_infos, 'train_metrics': train_metrics,
                        'sampling_metrics': sampling_metrics, 'visualization_tools': visualization_tools,
                        'extra_features': extra_features, 'domain_features': domain_features}
    elif dataset_config["name"] == 'transaction':
        from src.datasets.transaction_dataset import TransactionDataModule, TransactionDatasetInfos
        from src.metrics.transaction_metrics import TransactionSamplingMetrics
        datamodule = TransactionDataModule(cfg)
        dataset_infos = TransactionDatasetInfos(datamodule, cfg)
        train_metrics = TrainAbstractMetricsDiscrete() if cfg.model.type == 'discrete' else TrainAbstractMetrics()
        sampling_metrics = TransactionSamplingMetrics(datamodule, dataset_infos) # No longer a placeholder
        visualization_tools = NonMolecularVisualization(dataset_infos=dataset_infos)
    ...
    early_stopping_callback = EarlyStopping(monitor='val/epoch_CE_loss', patience=10, mode='min')
