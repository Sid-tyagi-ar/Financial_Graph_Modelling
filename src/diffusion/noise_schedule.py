...
class PredefinedNoiseScheduleDiscrete(torch.nn.Module):
    ...
    def __init__(self, noise_schedule, timesteps):
        ...
        elif noise_schedule == 'polynomial_2':
            beta_start = 1e-4
            beta_end = 0.02
...
class BlockDiagonalDiscreteUniformTransition:
    def __init__(self, x_classes: int, e_classes: int, y_feature_dims: list):
        self.X_classes = x_classes
        self.E_classes = e_classes
        self.y_feature_dims = y_feature_dims
        self.y_classes = sum(y_feature_dims)
        ...
        # Create block-diagonal u_y
        self.u_y = torch.zeros(1, self.y_classes, self.y_classes)
        if self.y_classes > 0:
            current_dim = 0
            for dim in self.y_feature_dims:
                self.u_y[:, current_dim:current_dim+dim, current_dim:current_dim+dim] = 1.0 / dim
    def get_Qt(self, beta_t, device):
        """ Returns one-step transition matrices for X, E and y, from step t - 1 to step t.
        Qt = (1 - beta_t) * I + beta_t / K
    ...
    def get_Qt_bar(self, alpha_bar_t, device):
        """ Returns t-step transition matrices for X, E and y, from step 0 to step t.
        Qt = prod(1 - beta_t) * I + (1 - prod(1 - beta_t)) / K
